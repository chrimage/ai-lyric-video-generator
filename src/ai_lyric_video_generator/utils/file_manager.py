...
from ai_lyric_video_generator.utils.utils import logger, FileOperationError # Assuming logger/FileOperationError are in utils.py
from ai_lyric_video_generator.config import config
# Import SongInfo for type hinting
...
class SongDirectory:
    ...
    def finalize_directory(self, song_info: SongInfo) -> str:
        """
        Finalize directory creation using official song info

        Args:
            song_info: SongInfo object with song metadata

        Returns:
            Path to the final song directory
        """
        # Extract artist and title using attribute access
        # title and artists are required fields in SongInfo dataclass
        title = song_info.title
        artists = song_info.artists
        artist_str = ', '.join(artists) if artists else 'Unknown Artist' # Keep fallback just in case
