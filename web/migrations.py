...
# Add the parent directory to the path so we can import from web.config
...
def add_video_columns():
    ...
    if Config.SQLALCHEMY_DATABASE_URI.startswith('sqlite:///'):
        ...
        if not os.path.isabs(db_name):
            # If relative, it's in the instance folder
            db_path = os.path.join(parent_dir, 'instance', db_name)
        else:
            db_path = db_name
    else:
